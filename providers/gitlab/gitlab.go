...
// NewClient returns a new Gitlab API client instance
func NewClient(token string) *Client {
	log.Printf("[DEBUG] [gitlab] Client creation : %s", token)
	...
	log.Printf("[DEBUG] [gitlab] Client created : %v", client)
	return client
}
...
func (c *Client) SetupHeaders(request *http.Request) {
	...
	if len(c.Token) > 0 {
		request.Header.Add("PRIVATE-TOKEN", c.Token)
	}
}
