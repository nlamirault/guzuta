...
docker run --rm \
       -v `pwd`:/tmp/ \
       $REPO/release \
       gox "${OS_PLATFORM_ARG[@]}" "${OS_ARCH_ARG[@]}" -output="/tmp/guzuta_{{.OS}}-{{.Arch}}"
...
for BINARY in guzuta_*; do
    docker run --rm -e GITHUB_TOKEN -v `pwd`:/go/src/github.com/nlamirault/guzuta \
